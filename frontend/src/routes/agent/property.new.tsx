...
export const Route = createFileRoute("/agent/property/new")({
...
async function compressImage(file: File): Promise<File> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      // Try reducing quality until under 500KB
      let quality = 0.85;
      const tryCompress = () => {
        canvas.toBlob((blob) => {
...
function NewProperty() {
  ...
  const handleImages = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newFiles = [...images, ...compressed];
    setImages(newFiles);
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-4xl">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5">
          <h3 className="font-semibold text-brand-ink border-b border-slate-100 pb-3 mb-4">
            Property Images <span className="text-slate-400 font-normal text-xs">(max 10 · auto-compressed to ≤500KB)</span>
          </h3>
        ...
        <div className="flex gap-3 pb-6">
          ...
          <button type="button" onClick={() => navigate({ to: backTo })} className="h-11 rounded-xl border border-slate-200 px-6 text-sm font-medium text-slate-600 hover:bg-slate-50 transition-colors">
            Cancel
          </button>
        </div>
      </form>
